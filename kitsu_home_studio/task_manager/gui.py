...
from kitsu_home_studio.shared.kitsu_auth import connect_to_kitsu, load_credentials, clear_credentials
from kitsu_home_studio.task_manager.kitsu_utils import get_user_projects, get_user_tasks_for_project, get_preview_thumbnail, clean_up_thumbnails, get_user_avatar, get_project_short_name, get_task_short_name
from kitsu_home_studio.task_manager.software_utils import clean_up_temp_files, print_context_from_json
    ...
    def contextMenuEvent(self, event):
        if self.tasks_list.underMouse():
            ...
            if action == action_view_details:
                ...
                if selected_task:
                    self.save_task_context(selected_task)
                    launch_krita(self.software_availability["Krita"], selected_task)
                    print_context_from_json()
